...
//fetch.php
require('../config.php');
$connect = mysqli_connect("localhost", "root", "", "milky");

if(isset($_POST["query"]))
{
 $search = mysqli_real_escape_string($connect, $_POST["query"]);
 $query = " SELECT * FROM user WHERE pseudo LIKE '%".$search."%'OR email LIKE '%".$search."%' OR confirmkey LIKE '%".$search."%'
 
 ";
}
else
{
 $query = " SELECT * FROM user ORDER BY confirmkey";
}
