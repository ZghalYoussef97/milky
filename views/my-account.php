...
$bdd = new PDO('mysql:host=localhost;dbname=milky', 'root', '');
session_start();
if(isset($_SESSION['confirmkey'])) {

$requser = $bdd->prepare("SELECT * FROM user WHERE confirmkey = ?");
$requser->execute(array($_SESSION['confirmkey']));
$user = $requser->fetchAll();
//var_dump($user);
}
$id=$_SESSION['confirmkey'];
if (isset($_POST['modifier']))
{//var_dump($_POST['modifier']);
 foreach ($user as $key ) {
       
    if ($_POST['pseudo']!=$key['pseudo'] AND $_POST['email']!=$key['email'] AND $_POST['mdp1']==$_POST['mdp2']) {
        # code...
 $pseudo=$_POST['pseudo'];
    $mail=$_POST['email'];
